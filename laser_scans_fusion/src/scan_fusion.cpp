...
void scan_fusion::scans_accumulator(std::vector<scan> &fused_ls,
                                    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int offset,
                                    tf::StampedTransform transform)
{
    float origin_distance = std::sqrt(std::pow(transform.getOrigin().x(), 2) +
                                      std::pow(transform.getOrigin().y(), 2)) +
                            0.1;
...
sensor_msgs::LaserScan scan_fusion::fuse_scans(std::vector<scan> fused_ls, fusion_config config)
{
    ...
    for (int i = 0; i < config.number_fused_points; i++)
    {
        ...
        while (theta_upper_range < fused_scans.angle_max && !exit_condition)
        {
            ...
            if (theta <= theta_upper_range && theta >= theta_lower_range)
            {
                ...
                else if ((abs(fused_ls[non_fused_scan_idx + 1].depth - fused_ls[non_fused_scan_idx].depth) > config.dis_threshold))
                    fused_scans.ranges[i] = fused_scans.range_max;
                else
                {
                    fused_scans.ranges[i] = (theta - theta_lower_range) * (fused_ls[non_fused_scan_idx + 1].depth - fused_ls[non_fused_scan_idx].depth) /
            ...
            else
            {
                ...
                theta_upper_range = fused_ls[std::min(non_fused_scan_idx + 1, static_cast<int>(fused_ls.size() - 1))].yaw;
            }
        }
    }
return fused_scans;
