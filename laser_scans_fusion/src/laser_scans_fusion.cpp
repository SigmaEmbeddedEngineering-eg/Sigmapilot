...
{
	this->node_.getParam(ros::this_node::getName() + "/fusion_frame",
						 this->config.fusion_frame);
	this->node_.getParam(ros::this_node::getName() + "/number_fused_points",
						 this->config.number_fused_points);
	this->node_.getParam(ros::this_node::getName() + "/range_min",
						 this->config.range_min);
	this->node_.getParam(ros::this_node::getName() + "/dis_threshold",
						 this->config.dis_threshold);
	this->node_.getParam(ros::this_node::getName() + "/tf_listener_rate",
						 this->config.tf_listener_rate);

	std::string input_topic1, input_topic2, fused_topic;
	this->node_.getParam(ros::this_node::getName() + "/input_topic1",
						 input_topic1);
	this->node_.getParam(ros::this_node::getName() + "/input_topic2",
						 input_topic2);
	this->node_.getParam(ros::this_node::getName() + "/fused_topic",
						 fused_topic);
...
void Multi_Scan_Fusion::scanCallback(const sensor_msgs::LaserScan::ConstPtr &scan1, const sensor_msgs::LaserScan::ConstPtr &scan2)
{
	tf::StampedTransform transform, transform2;
	tfListener_.lookupTransform(this->config.fusion_frame, scan1->header.frame_id,
								ros::Time(0), transform);
	tfListener_.lookupTransform(this->config.fusion_frame, scan2->header.frame_id,
	...
	sf.scans_accumulator(fused_ls, temp_cloud, 0, transform);
	sf.scans_accumulator(fused_ls, temp_cloud2, fused_ls.size(), transform2);

	config.range_max = std::max(scan1->range_max, scan2->range_max) +
					   ...
					   0.1;
	sensor_msgs::LaserScan fused_scans = sf.fuse_scans(fused_ls, config);
