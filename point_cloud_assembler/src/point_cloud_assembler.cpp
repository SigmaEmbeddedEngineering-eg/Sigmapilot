...
pointCloudAssembler::pointCloudAssembler()
{
  ...
  std::string pub_topicname = "assemblerMap";
	this->nh.getParam(ros::this_node::getName() + "/pub_topicname",
						 pub_topicname);
	this->nh.getParam(ros::this_node::getName() + "/voxel_size",
						 this->config.voxel_size);             
	this->nh.getParam(ros::this_node::getName() + "/tf_duration",
						 this->config.tf_duration);             
	this->nh.getParam(ros::this_node::getName() + "/odom_frame",
						 this->config.odom_frame);             
...
void pointCloudAssembler::cloud_cb(const PointCloud::ConstPtr &input)
{
  ...
  tfListener_.lookupTransform(this->config.odom_frame, input->header.frame_id,
  ...
  this->cloud_accumilated.width =
      this->cloud_accumilated.width + cloud_trans.width;
  this->cloud_accumilated.height = 1;
  this->cloud_accumilated.points.insert(this->cloud_accumilated.points.end(),
                                        cloud_trans.points.begin(),
                                        cloud_trans.points.end());

  
  std::cout << "PointCloud after filtering: " << downsampledPC->width << "  "
            << downsampledPC->height
            << "  before filtering:" << input->width * input->height
            << "  accumilative filtering:"
            << this->cloud_accumilated.width * this->cloud_accumilated.height
            << std::endl;
  
  // down sample the assembled point cloud.
  PointCloud::Ptr accumilatedPC(new pcl::PointCloud<pcl::PointXYZRGB>);
  PointCloud::Ptr downsampledAccumilatedPC(
